...
void InvalidateDataCache (void)
{
	...
	for (register unsigned nSet = 0; nSet < L1_DATA_CACHE_SETS; nSet++)
	{
		...
		{
			register unsigned int nSetWayLevel =   nWay << L1_SETWAY_WAY_SHIFT
	...
	for (register unsigned nSet = 0; nSet < L2_CACHE_SETS; nSet++)
	{
		...
		{
			register unsigned int nSetWayLevel =   nWay << L2_SETWAY_WAY_SHIFT
...
void InvalidateDataCacheRange (unsigned int nAddress, unsigned int nLength)
{
...
void CleanDataCacheRange (unsigned int nAddress, unsigned int nLength)
{
	nLength += DATA_CACHE_LINE_LENGTH_MIN;
...
void CleanAndInvalidateDataCacheRange (unsigned int nAddress, unsigned int nLength)
{
	nLength += DATA_CACHE_LINE_LENGTH_MIN;
